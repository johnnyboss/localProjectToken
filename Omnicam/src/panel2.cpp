...
void Panel2::create()
{
    ...
    setup(0, (ofGetHeight()-80)/2.0 + 80, ofGetWidth(), (ofGetHeight()-80)/2.0);

    slider aux2;
    aux2.create(panelArea.x + 10, panelArea.y + 20, panelArea.width - 20, 80, 0.5, "omnicam2pos");
    ...
    aux.create(120 + panelArea.x, 5 + panelArea.y, "omnicam2motorbattery", "Motor", 0);
    labels.push_back(aux);//1
    aux.create(180 + panelArea.x, 5 + panelArea.y, "omnicam2electbattery", "Electronics", 0);
    labels.push_back(aux);//2
    aux.create(280 + panelArea.x, 5 + panelArea.y, "omnicam2temp1", "M1Temperature", 0);
    labels.push_back(aux);//3
    aux.create(390 + panelArea.x, 5 + panelArea.y, "omnicam2temp2", "M2Temperature", 0);
    ...
    posGraph.create(panelArea.x + 10, panelArea.y + 110, "omnicam2graph");
...
void Panel2::dealwithtouches(int x, int y, int button)
{
    ...
    if(isClicked(x,y) && buttons[18].isActive() && !buttons[3].isClicked(x,y) && !block)
    {
        ...
        if(buttons[15].isActive() && button == 1)
        {
            ...
            if(keypad.exit)
            {
                ...
                keypad.value = 0;
                toSend = true;
                msg = "omnicam2vmax_"+ofToString(vMax);
        ...
        if(buttons[16].isActive() && button == 1)
        {
            ...
            if(keypad.exit)
            {
                ...
                keypad.value = 0;
                toSend = true;
                msg = "omnicam2vmin_"+ofToString(vMin);
        ...
        if(buttons[17].isActive() && button == 1)
        {
            ...
            if(keypad.exit)
            {
                ...
                keypad.value = 0;
                toSend = true;
                msg = "omnicam2posvelmax_"+ofToString(goSpeed);
            }
        }
...
void Panel2::update(string message)
{
    ...
    for (int j=0; j < msg1.size(); j++)
    {
        ...
        if(msgs[0] == "omnicam2actuator")
        {
            actuator = ofToFloat(msgs[1]);
            //posGraph.updateActuator(ofToFloat(msgs[1]));
        }

        if(msgs[0] ==  "omnicam2servopower")
        {
            if(ofToInt(msgs[1]) == 0)
                servopower = false;
            else
